...
/* ---------------------- FAIRNESS (JAARBALANS) ---------------------- */
/** Bepaalt gemiddelden & achterstanden met uitsluiting van niet-deelnemers per categorie. */
function buildFairness(counters){
  const isGenActive = (d)=>{
    const c=counters?.[d]||ZERO();
    return (c.iw+c.iwe+c.lw+c.lwe) > 0;
  };
  const isCarActive = (d)=>{
    const c=counters?.[d]||ZERO();
    return (c.cw+c.cwe) > 0;
  ...
  for(const d of genPool){
    const c=counters?.[d]||ZERO();
    mean.iw  += c.iw;  mean.iwe += c.iwe; mean.lw  += c.lw;  mean.lwe += c.lwe;
  }
  if(genPool.length){
    mean.iw/=genPool.length; mean.iwe/=genPool.length; mean.lw/=genPool.length; mean.lwe/=genPool.length;
  }
  for(const d of carPool){
    const c=counters?.[d]||ZERO();
    mean.cw  += c.cw;  mean.cwe += c.cwe;
  }
  if(carPool.length){
    mean.cw/=carPool.length; mean.cwe/=carPool.length;
  }
  ...
  for(const d of ALL_DOCTORS){
    ...
    def[d] = {
      ...
      cw:  mean.cw  - c.cw,
      cwe: mean.cwe - c.cwe,
      cardioTotal: (mean.cw + mean.cwe) - (c.cw + c.cwe),
    };
  }
